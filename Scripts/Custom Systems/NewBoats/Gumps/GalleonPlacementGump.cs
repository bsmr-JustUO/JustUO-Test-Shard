...
namespace Server.Multis
{
    public class GalleonPlacementGump : Gump
    {
		...
		private BaseDockedGalleon m_DockedGalleon;
        ...
        public override void OnResponse(NetState state, RelayInfo info)
        {
            if (info.ButtonID == 1)
			{
				...
				else if (m_DockedGalleon != null)
				{
					m_DockedGalleon.PlacementDirection(m_From, Direction.West);
				}
			}
			else if (info.ButtonID == 2)
			{
				...
				{
					m_DockedGalleon.PlacementDirection(m_From, Direction.North);
				}
			}
            else if (info.ButtonID == 3)
			{
				...
				{
					m_DockedGalleon.PlacementDirection(m_From, Direction.South);
				}
			}
            else if (info.ButtonID == 4)
			{
				...
				{
					m_DockedGalleon.PlacementDirection(m_From, Direction.East);
				}
			}
        }
    }
